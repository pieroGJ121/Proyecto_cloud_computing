...
class ProyectTests(unittest.TestCase):
    def setUp(self):
        ...
        response_user = self.client.post("/create", json=self.new_usuario)
    ...
    def test_profile_patch_success(self):
        ...
        response = self.client.patch(
            "/profile", headers=self.headers, json=self.new_usuario
        )
        data = json.loads(response.data)
    ...
    def test_profile_patch_fail(self):
        ...
        response = self.client.patch("/profile", headers=self.headers, json=temp_user)
        data = json.loads(response.data)
    ...
    def test_profile_delete_fail(self):
        self.headers["X-ACCESS-TOKEN"] = self.user_valid_token
        response = self.client.delete("/profile/failure", headers=self.headers)
        data = json.loads(response.data)
